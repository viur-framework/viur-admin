...
class TextEdit(QtWidgets.QMainWindow):
	...
	def setupEditActions(self):
		...
		self.actionUndo = QtWidgets.QAction(QtGui.QIcon(rsrcPath + '/../undo.svg'), "&Undo", self,
		                                shortcut=QtGui.QKeySequence.Undo)
		tb.addAction(self.actionUndo)
		self.actionRedo = QtWidgets.QAction(QtGui.QIcon(rsrcPath + '/../redo.svg'),
	...
	def setupInsertActions(self):
		...
		if "a" in self.validHtml["validTags"]:
			self.actionInsertLink = QtWidgets.QAction(QtGui.QIcon(rsrcPath + '/link.png'), "&Link",
			                                      self)
		...
		if "img" in self.validHtml["validTags"]:
			self.actionInsertImage = QtWidgets.QAction(QtGui.QIcon(rsrcPath + '/image_add.png'),
			                                       "&Image", self)
	...
	def setupTextActions(self):
		...
		if "i" in self.validHtml["validTags"]:
			self.actionTextItalic = QtWidgets.QAction(QtGui.QIcon(rsrcPath + '/italic.png'),
			                                      "&Italic", self,
			                                      priority=QtWidgets.QAction.LowPriority,
		...
		if "p" in self.validHtml["validAttrs"].keys() and "align" in self.validHtml["validAttrs"]["p"]:
			...
			if QtWidgets.QApplication.isLeftToRight():
				...
				self.actionAlignCenter = QtWidgets.QAction(
					QtGui.QIcon(rsrcPath + '/aligncenter.png'),
					"C&enter", grp)
		...
		if "font" in self.validHtml["validAttrs"].keys() and "color" in self.validHtml["validAttrs"][
			"font"]:
		...
		if "ul" in self.validHtml["validTags"]:
			self.actionBulletList = QtWidgets.QAction(QtGui.QIcon(':icons/actions/text/bullet.png'),
			                                      "Bullet",
			                                      self,
		...
		if "ol" in self.validHtml["validTags"]:
			self.actionNumberedList = QtWidgets.QAction(QtGui.QIcon(':icons/actions/text/numbered.png'),
			                                        "Numbered",
			                                        self,
	...
	def textColor(self):
		col = QtWidgets.QColorDialog.getColor(self.ui.textEdit.textColor(), self)
		if not col.isValid():
			return
	...
	def insertLink(self, *args, **kwargs):
		(dest, okay) = QtWidgets.QInputDialog.getText(self,
		                                          QtCore.QCoreApplication.translate("TextEditBone",
		                                                                            "Specify target"),
		                                          QtCore.QCoreApplication.translate("TextEditBone",
		                                                                            "Link target:"))
	...
	def insertHeading(self, lvl):
		...
		if not cursor.hasSelection():
			cursor.select(QtGui.QTextCursor.WordUnderCursor)
		txt = cursor.selectedText()
		cursor.insertHtml("<h%s>%s</h%s> " % (
			lvl, txt, lvl))  # The tailing whitespace is nessesary to reset the style afterwards
...
class DocumentToHtml:
	...
	def processBlock(self, block, document):
		...
		if block.textList() is not None:
			...
			if block.textList().itemNumber(
				block) == block.textList().count() - 1:  # This was the last one
				self.closeTag("li")
				self.closeTag(("ul", "ol"))
		if not self.isTagOpen("ul") and not self.isTagOpen("ol"):
			# Each block<=>paragraph (unless we have a list)
	...
	def processFragment(self, fragment, block, document):
		...
		if txtFormat.objectType() == txtFormat.ImageObject:
			# FIXME: Why isnt txtFormat an instance of QtWidgets.QTextImageFormat in this case?
		...
		# Check for hrefs
		if self.isTagOpen(
			"a") and not txtFormat.anchorHref():  # This a has been closed recently
			self.closeTag("a")
		elif self.isTagOpen("a") and self.getLastInternalInfo(
			"a") != txtFormat.anchorHref():  # The href has changed
			self.closeTag("a")
			self.openTag("a", tagArgs={"href": txtFormat.anchorHref()},
			             internalInfo=txtFormat.anchorHref())
		...
		if not self.isTagOpen("a"):  # Dont apply QTs style to hrefs
			...
			if txtFormat.fontWeight() in self.headingMap.keys() and self.isTagOpen(
				("h1", "h2", "h3")):
			...
			if txtFormat.fontWeight() in self.headingMap.keys() and all(
				...
				if color.red() or color.green() or color.blue():  # It's not black
					if not self.isTagOpen("font") or self.getLastInternalInfo(
						"font") != colorStr:
						self.openTag("font", tagArgs={"color": colorStr},
						             internalInfo=colorStr)
	...
	def closeTag(self, tag):
		...
		for t in self.openTags[:: -1]:
			deletedTags += 1
			self.res += "</%s>" % t[0]
			if (isinstance(tag, str) and t[0] == tag) or (
					isinstance(tag, tuple) and t[0] in tag):
				break
