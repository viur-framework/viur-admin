...
class GraphPlotter(QtWidgets.QWidget):
	...
	def paintEvent(self, event):
		...
		for y in range( len(self.data.keys())-1, -1, -1): #X-Axis description
			...
			qp.setPen( QtGui.QColor( self.txtColor ) )
			qp.drawText( QtCore.QRect( xPos-40, event.rect().height()-45, 80, 20 ), QtCore.Qt.AlignCenter, (dtNow-timedelta(days=y)).strftime("%d.%m.%Y") )
			xPos += xStep
		visitSources = []
		for x in range(0, 8):
		...
		for type in visitSources:
			...
			for x in range(7, -1, -1):
				...
				if (self.mousePos[0]-xPos)*(self.mousePos[0]-xPos)+(self.mousePos[1]-pointHeight)*(self.mousePos[1]-pointHeight) < 20:
		...
		if highlightStats:
			...
			qp.setPen( QtGui.QColor( self.graphColor ) )
			if type=="total":
				txt = QtCore.QCoreApplication.translate("Analytics", "%s (total): %s") % ((dtNow-timedelta(days=day)).strftime("%d.%m.%Y"), visists )
			else:
				txt = QtCore.QCoreApplication.translate("Analytics", "%s via %s: %s") % ((dtNow-timedelta(days=day)).strftime("%d.%m.%Y"), type, visists )
...
class AnalyticsTableModel( QtCore.QAbstractTableModel ):
	...
	def headerData(self, col, orientation, role):
		if orientation == QtCore.Qt.Horizontal and role == QtCore.Qt.DisplayRole:
			...
			return ( (datetime.now()-timedelta( days=col )).strftime( QtCore.QCoreApplication.translate("Analytics", "%d.%m.%Y") ) )
		return None
...
class AnalytisWidget(QtWidgets.QWidget):
	...
	def __init__(self):
		...
		if "configuration" in conf.serverConfig.keys() and "analyticsKey" in conf.serverConfig["configuration"].keys():
			...
			self.request = NetworkService.request( "https://www.google.com/accounts/ClientLogin", credStr.encode("UTF-8")  )
			...
			for x in range(1, 8):
				...
				url = "https://www.google.com/analytics/feeds/data?sort=-ga:visits&max-results=50&start-date=%s&ids=%s&metrics=ga:visits&end-date=%s&dimensions=ga:source" % ( (dtNow-tdStart).strftime("%Y-%m-%d"), self.uaKey, (dtNow-tdEnd).strftime("%Y-%m-%d") )
	...
	def fetchNext(self):
		...
		if not self.request:
			if len( self.queue )>0:
				self.request = NetworkService.request( self.queue[-1][1], extraHeaders={"Authorization": "GoogleLogin auth="+self.sessionID} )
		...
		try:
			self.data[ currentRequestData[0] ]["total"] = int(dom.getElementsByTagName('dxp:aggregates')[0].getElementsByTagName("dxp:metric")[0].getAttribute("value"))
