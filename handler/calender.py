...
class CalenderWidget( ListWidget ):
	...
	def updateDateFilter(self):
		...
		if self.currentFilterMode=="none":
			...
			filter["startdate$lt"] = QtCore.QDate( self.deFilter.date().year(), self.deFilter.date().month(), 1).addDays(33).toString("01.MM.yyyy")
		elif self.currentFilterMode=="day":
...
class CalenderCoreHandler( WidgetHandler ): #EntryHandler
	...
	def __init__( self, modul,  *args, **kwargs ):
		...
		if config["icon"]:
			...
			icon = loadIcon( None )
		super( CalenderCoreHandler, self ).__init__( widgetGen, descr=config["name"], icon=icon, vanishOnClose=False, *args, **kwargs )
