...
class TextEdit(QtGui.QMainWindow):
	...
	def o2n_textEdit_contextMenuEvent(self, event):
		...
		if self.textEdit.textCursor().currentTable():
			...
			insertColumBefore = insertMenu.addAction( QtCore.QCoreApplication.translate("DocumentEditBone", "Column before") )
			insertColumAfter = insertMenu.addAction(QtCore.QCoreApplication.translate("DocumentEditBone", "Column after"))
...
#		tb.addAction(self.actionPaste)
	...
	def insertImage(self, *args, **kwargs ):
		...
		event.emit( QtCore.SIGNAL('requestTreeItemBoneSelection(PyQt_PyObject,PyQt_PyObject,PyQt_PyObject,PyQt_PyObject,PyQt_PyObject,PyQt_PyObject)'), queue, "file", "textEdit", skelStructure, None, self.setFileSelection )
	...
	def setaSelection(self, selection):
		...
		document.addResource( document.ImageResource, QtCore.QUrl("/file/view/%s/%s" % ( file["dlkey"], file["name"]) ), img )
...
class FileEdit( QtGui.QWidget ):
	...
	def __init__(self, saveCallback, attrs, href, *args, **kwargs ):
		...
		if "src" in attrs.keys():
			src = attrs["src"]
			if "/file/view" in src:
				self.fileDlKey = src[ src.find("/file/view/" )+len("/file/view/") : src.find( "/", src.find("/file/view" )+5+len("/file/view") ) ]
				self.fileName = src[ src.find("/", src.find("/file/view/" )+len("/file/view/")+5)+1 : src.find( "?", src.find("/", src.find("/file/view/" )+len("/file/view/")+5) ) ]
				if "?id=" in src:
...
class HeaderBlock( HtmlBlock ):
	...
	def getEditor( self, parent  ):
		...
		html = "<html><head><base href=\"%s\"></head><body>%s</body></html>" % ( NetworkService.url,parent.serialize( self )) 
		currentEditor.setHtml( html )
		return( currentEditor )

	def doubleClicked(self):
		txt, okay = QtGui.QInputDialog.getText(None, QtCore.QCoreApplication.translate("DocumentEditBone", "Change caption"), QtCore.QCoreApplication.translate("DocumentEditBone", "Insert new caption"), text=self.content)
		if okay and txt:
			self.content = txt
			self.setText(0, self.content)
...
class Parser( html.parser.HTMLParser ):
	...
	def handle_starttag(self, tag, attrs):
		...
		if tag!="a" or self.openTables:
			...
			self.lastHRef = ( [ (k, v) for (k, v) in attrs if k[0:2].lower() != 'on' and v[0:10].lower() != 'javascript'] )
...
class HtmlSerializer( html.parser.HTMLParser ):
	...
	validTags = ('font','b', 'a', 'i', 'u', 'span', 'div','p', 'img','ul','li','acronym','h1','h2','h3','h4','h5','h6', 'table', 'tr', 'td', 'th', 'br', 'hr', 'strong')
...
class DocEdit( QtGui.QWidget ):
	...
	def on_btnNewHeading_released(self, *args, **kwargs):
		txt, okay = QtGui.QInputDialog.getText(self, QtCore.QCoreApplication.translate("DocumentEditBone", "New caption"), QtCore.QCoreApplication.translate("DocumentEditBone", "Insert new caption"))
		if okay:
	...
	def getBreadCrumb( self ):
		return( QtCore.QCoreApplication.translate("DocumentEditBone", "Document editor"), QtGui.QIcon( QtGui.QPixmap( "icons/actions/text-edit.png" ) ) )
